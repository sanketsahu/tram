...
Not a daemon, not a resident multi-tenant server, no process management. Each `jetplane`
...
```
jetplane dev                     # detect framework, ensure deps, run cache-backed dev server
jetplane dev --framework vite    # force framework
jetplane install                 # just resolve + install + distill vendor image (optional)

# escape hatch: the Vite adapter is a plugin, usable without the runner
# vite.config.ts
import { jetplane } from 'jetplane/vite'
export default { plugins: [jetplane()] }
```
...
```
 CLI              jetplane dev  (detect framework: vite | expo | next)
   |
 ...
 Core             ┌───────────────────────────────────────────────┐
                  ...
                  └───────────────────────────────────────────────┘
   |
 System           ~/.jetplane/store   (content-addressed, shared across ALL projects)
                  ~/.jetplane/images  (packed base images, mmap'd read-only)
```

The **value is the core + the shared cache**, not the CLI. Adapters are small; each
framework is "how do I get this framework to fetch modules from the Jetplane core."

---

## 3. Jetplane owns install

No manual `npm install`. Consequences:

- **Pluggable installer backend.** jetplane picks the fastest available (bun > pnpm > npm),
...
- **jetplane controls the lockfile hash**, which is the `manifest` key (H2). No
  reverse-engineering of a package manager's layout.
- **Install is when the vendor image is built/warmed.** First `jetplane dev` is already hot.
...
| `jetplane` re-install (new/changed dep) | lockfileHash → new manifest | transform only changed modules; re-pack |
...
- **mmap the packed vendor image read-only** → OS page cache dedupes physical pages
  across every independent `jetplane` process. This is how the "2.1 GB → 11 MB for 200 envs"
...
3. **Vite adapter** (plugin): serve vendor from the Jetplane image (externalized from Vite),
   let Vite own the app layer + HMR. `jetplane dev` detection.
