...
// pointing cacheDir at ~/.tram/vite/<lockHash>, every project/run with the same deps
...
// vite.config.ts without the `tram` runner.
...
export function tramVite(opts: TramViteOptions = {}) {
  ...
  return {
    ...
    config(_userConfig: any, _env: any) {
      ...
      log(`tram: lockHash=${lh}`)
      log(`tram: vendor cache ${warm ? 'WARM (reuse shared prebundle)' : 'COLD (build shared prebundle)'} -> ${shared}`)
      return {
        cacheDir: shared,
    ...
    configResolved(resolved: any) {
      log(`tram: cacheDir resolved to ${resolved.cacheDir}`)
    },
  }
}

// Start a real Vite dev server for `projectDir` with the tram plugin injected,
