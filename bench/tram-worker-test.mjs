...
const tram = require('/Users/sanketsahu/projects/tram/src/tram-transformer.cjs')
...
execSync('rm -rf ~/.tram/tstore', { shell: '/bin/zsh' })
const count = () => { try { return execSync('find ~/.tram/tstore -type f | wc -l', { shell: '/bin/zsh' }).toString().trim() } catch { return '0' } }

const relFile = 'node_modules/react-native/Libraries/Components/View/View.js'
const src = fs.readFileSync(`${APP}/${relFile}`)

// 1) cold
const r1 = await tram.transform(config, APP, `${APP}/${relFile}`, src, options)
...
// 2) warm, same root
const before = count()
const r2 = await tram.transform(config, APP, `${APP}/${relFile}`, src, options)
...
const b4 = count()
const r3 = await tram.transform(config, ROOTB, `${ROOTB}/${relFile}`, src, options)
